...
**Modo Podcast validado em produção e change arquivada (09/09/2026):** depois do fix de timeout de upload (ver bloco abaixo), o usuário testou em produção e confirmou sucesso em três frentes — upload de arquivo grande, pipeline completo via link do YouTube em modo Podcast, e ausência de regressão em Shorts/Palavra Completa. A change OpenSpec `add-podcast-clipping-mode` foi arquivada (`openspec/changes/archive/2026-09-09-add-podcast-clipping-mode/`) e os deltas sincronizados para os specs principais (`clip-studio/video-library`, `clip-studio/youtube-ingestion` modificados; `clip-studio/video-upload-ingestion`, `podcast-clipping` criados como capabilities novas). Única pendência restante (não bloqueante, registrada na tasks.md arquivada como 7.4): calibração fina dos critérios de "destaque de podcast" nos prompts, validável só com mais episódios reais e/ou uma auditoria do agente `clipador`.

**Bug corrigido — upload de arquivo travava com 408 antes de completar em arquivos grandes (09/09/2026):** o primeiro teste real de upload de arquivo (pendência 6.5/7.2 da change acima) expôs dois problemas que o `next build`/`eslint` não pegam, porque são de infraestrutura de servidor, não de lógica de aplicação:
1. **`next start` roda sobre o `http.Server` padrão do Node, que tem `server.requestTimeout` default de 5 minutos** desde o Node 18 (Next.js nunca sobrescreve isso) — um upload de vídeo de vários GB numa conexão doméstica normal rotineiramente demora mais que isso, então a conexão era derrubada com HTTP 408 antes do corpo da requisição terminar de chegar, mesmo com o proxy de streaming já implementado corretamente. Não existe opção no `next.config.ts` pra isso — a única forma de mudar é um **custom server**. Fix: novo `clip-studio/server.js` (usa `next()` + `http.createServer` manualmente, com `server.requestTimeout = 0` pra desabilitar o timeout), `package.json` (`"start": "node server.js"` em vez de `"start": "next start"`) e `Dockerfile` (copia `server.js` pro estágio final).
2. **O proxy do lado do Next.js pro n8n tinha exatamente o mesmo problema, só que no n8n** — mesmo com o `server.js` acima resolvendo o lado Next.js, um teste com upload throttled reproduziu o 408 idêntico contra o n8n (que também roda um processo Node com o mesmo default de 5min, sem nenhuma flag exposta pra mudar isso, e é um pacote de terceiro — não dá pra fazer patch do bootstrap dele sem quebrar a cada atualização). Fix: `src/app/api/submissions/upload/route.ts` parou de repassar o `ReadableStream` da requisição ao vivo (`fetch(..., {body: stream, duplex:"half"})`) direto pro webhook n8n — agora bufferiza o corpo recebido num arquivo temporário local (`mkdtemp`/`pipeline`/`createWriteStream`) antes de reenviar via `createReadStream`. Isso significa que a perna Next.js→n8n é sempre uma transferência rápida e interna à VPS (medido: 2.2GB em ~40s) independente de quão lenta seja a conexão de quem está enviando o arquivo — o lado lento (browser→Next.js) já não tem mais timeout (fix #1), e o lado rápido (Next.js→n8n, sempre <5min) nunca bate no timeout do n8n. Arquivo temporário sempre limpo em `finally` (`rm -rf`, best-effort).

**Deploy em produção confirmado (09/09/2026):** verificado via SSH (`ssh vps-mcobo`) que o código (schema, rota, UI, `server.js`, `Dockerfile`) já estava idêntico byte-a-byte ao repositório local, o container `clipstudio-web-1` já rodando com a imagem rebuildada, e as migrations (`add_podcast_mode`, `submission_youtube_url_nullable`) já aplicadas no Postgres — o deploy tinha sido feito numa sessão anterior à cópia+commit destes arquivos nesta sessão. Nada precisou ser refeito, só confirmado.

---

**Modo Podcast adicionado (08/09/2026, change OpenSpec `add-podcast-clipping-mode`):** a pedido do usuário, o Clip Studio ganhou um terceiro modo de submissão, independente de Shorts e Palavra Completa, pra cortar podcasts de qualquer tipo (conteúdo sem fases de culto). Implementado ponta a ponta nesta sessão — schema, UI, dois workflows n8n novos.

- **Clip Studio:** `SubmissionMode` ganhou o valor `PODCAST` (terceiro valor independente, não um sub-modo de Shorts/Palavra Completa — decisão explícita do usuário na fase de proposta). `Submission.youtubeUrl` virou opcional (`String?`) pra suportar o novo caminho de upload de arquivo direto. `SubmitForm.tsx` ganhou um seletor "Tipo de conteúdo" (Pregação/Podcast, default Pregação) — quando Podcast está selecionado, o toggle "Modo Palavra Completa" some (não se aplica a podcast) e o modo fica fixo em `PODCAST`; e uma aba nova "Enviar arquivo" ao lado de "Link do YouTube", disponível pra qualquer tipo de conteúdo, não só podcast. O upload de arquivo é feito sem multipart/form-data — metadados (título, modo, nome do arquivo) na query string, corpo da requisição = bytes crus do arquivo — permitindo streaming de ponta a ponta (`request.body` do Next.js repassado direto pro webhook n8n via `fetch(..., {body: stream, duplex:"half"})`, nunca bufferizado inteiro em memória, mesmo cuidado já documentado pro download do lado do n8n). Nova rota `POST /api/submissions/upload` cria a submissão já com status `BAIXANDO` (sem passar pela fila `FILA`→`dispatchNextIfIdle()`, já que aqui o "download" é o próprio upload já em andamento na requisição) e delega a transição pra `PROCESSANDO`/`CONCLUIDO` ao mesmo callback (`/api/webhooks/n8n/ingestion`) e poller (`isOriginalArchived`) já usados pelo caminho de link do YouTube. Reprocessar (`/api/submissions/[id]/reprocess`) uma submissão feita por upload é bloqueado explicitamente com uma mensagem clara — não há como reprocessar automaticamente, os bytes originais nunca foram persistidos pelo Clip Studio, só streamados através.

- **n8n — workflow de ingestão** (`Clip Studio — Integração N8N`, `mfqp4D5HKs0MNhv1`): o node IF "Rotear por Modo" (que já existia pra Shorts/Palavra Completa) ganhou um segundo IF encadeado na saída falsa ("Rotear Podcast"), checando `mode==PODCAST` — os ramos `SHORTS`/`PALAVRA_COMPLETA` continuam **byte-a-byte idênticos** ao que já existia (confirmado via diff programático nó a nó contra o estado anterior à mudança). O ramo `PODCAST` sobe o vídeo pra `Videos-Cortes/Podcast` (pasta criada sob demanda, idempotente) e dispara o novo workflow de Podcast. Além disso, o workflow ganhou um **segundo trigger novo**, `Webhook Ingerir Video Upload` (path `clip-studio/ingest/upload`, `options.binaryData:true`), pra receber os arquivos enviados via upload direto: recebe o corpo bruto como binary data, grava em disco local (`readWriteFile`), resolve a pasta de fila certa por `mode` (lookup direto, sem branching — `{SHORTS:'Videos-Cortes', PALAVRA_COMPLETA:'Videos-Cortes/PalavraCompleta', PODCAST:'Videos-Cortes/Podcast'}[mode]`), sobe em blocos via sessão de upload + PUT por chunk (mesmo padrão wget já usado no resto do pipeline, com o quoting seguro `"${PREFIX}"*`/`for part in "${PREFIX}"*` já aplicado desde o início — ver bug corrigido mais abaixo), chama o callback de sucesso, e dispara o workflow certo (`Disparar Pipeline Upload`, com `workflowId` resolvido dinamicamente por expressão a partir do `mode`, sem precisar de um switch de 3 ramos). Os webhooks utilitários `clip-studio/clips` e `clip-studio/videos/check-archived` (mesmo workflow) foram estendidos pra também listar/checar a saída do Podcast (`Videos-Cortes/Podcast/Cortes`, `Videos-Cortes/Podcast/Videos`).

- **Novo workflow n8n "YouTube Podcast — Extração de Highlights"** (`Zfwhv4pppJf3NhkR`, publicado, 37 nodes): construído nos mesmos moldes de "Blocos" — cópia estrutural completa (não referenciada, workflows n8n são documentos separados) até "Montar Clipes": mesma trava de execução **compartilhada** com "Blocos"/"Palavra Completa" (mesmo arquivo `/home/node/.n8n-files/.processing.lock`, pra nunca rodar 2 whisper.cpp simultâneos na VPS de 6 vCPUs — agora 3 pipelines competindo pelo mesmo lock), download via wget streaming, FFprobe+Whisper.cpp (`large-v3-turbo -t 6 -mc 0`), "Mesclar Pausas Curtas", "Dividir em Blocos", os 2 passes de IA, "Montar Clipes", "FFmpeg Cortar 9:16" com crop vertical (ao contrário de Palavra Completa, Podcast **mantém** o crop 9:16 — múltiplos clipes curtos, mesmo formato de Shorts). Diverge nos dois prompts de IA, reescritos do zero: `Preparar GPT — Analisar Blocos Podcast` substitui a REGRA DE EXCLUSÃO OBRIGATÓRIA por fase de culto (que não existe em podcast) por critérios de destaque de podcast (revelação/opinião forte, humor genuíno, debate/contraponto, insight prático, frase citável), mantendo a mesma REGRA DE CONSISTÊNCIA NUMÉRICA e o framing "FILTRO GROSSEIRO com nota BASE por critério" já corrigidos em "Blocos" (evita repetir o bug histórico de scoring degenerado). Conteúdo de baixo valor (publicidade lida do roteiro, transição vazia tipo "e aí, bora começar") é pontuado baixo **pelos próprios critérios** (sem gancho, sem emoção, sem impacto), não por uma regra categórica — decisão explícita porque não existe um sinal estrutural equivalente à "fase do culto" em podcast. `Preparar GPT — Seleção Final Podcast` mantém o mesmo checklist de cobertura por janelas de tempo, teto de 180s e gap mínimo de 15s já validados em "Blocos", sem a seção de fases excluídas (substituída por um parágrafo "QUALIDADE DO CONTEÚDO" com a mesma lógica não-categórica). "Montar Clipes Podcast" não tem a extração de nome do pregador (regex removida — não se aplica; o slug do arquivo usa só o título do clipe). Fila própria (`Videos-Cortes/Podcast`), self-chaining e Schedule Trigger próprio (a cada 30min, mesmo raciocínio de Palavra Completa — compete pelo lock, precisa reprocessar rápido quando ele libera).

- **Bug real encontrado e corrigido preventivamente durante a implementação:** o mesmo bug de quoting já corrigido em 08/09/2026 no upload do clipe final de Palavra Completa (`ls ${PREFIX}*` sem aspas quebra com espaço no nome do arquivo — ver seção "Bug corrigido — upload em chunks..." abaixo) foi aplicado desde o início em **todos** os comandos de upload em chunks novos desta change (ramo `PODCAST` do workflow de ingestão, webhook de upload de arquivo), mesmo nos casos onde o nome local do arquivo não tem espaço (usa só `submissionId`) — não é estritamente necessário nesses casos específicos, mas é mais seguro e consistente não depender dessa garantia implícita.

- **Validação local feita antes de publicar:** todos os 15 Code nodes novos/alterados (nos dois workflows) tiveram a sintaxe confirmada com `new Function()`; a lógica de normalização de entrada (título com caracteres especiais, os 3 modos, submissionId vazio) testada em runtime com dados fictícios; todos os 8 comandos bash novos/alterados passaram em `sh -n`; confirmado via diff programático nó a nó que os 20 nodes dos ramos `SHORTS`/`PALAVRA_COMPLETA` do workflow de ingestão ficaram byte-a-byte idênticos (só `Normalizar Entrada` mudou, esperado); `next build` e `eslint` (projeto inteiro) rodaram limpos duas vezes.

- **✅ Validado em produção (09/09/2026):** deploy feito, upload de arquivo grande testado com sucesso (depois do fix de timeout, ver bloco "Bug corrigido — upload de arquivo travava com 408" acima), pipeline completo via YouTube em modo Podcast testado com sucesso, e confirmado sem regressão em Shorts/Palavra Completa. Change arquivada. Única pendência remanescente: calibração fina dos critérios de "destaque de podcast" nos prompts — só validável com mais episódios reais e/ou uma auditoria do agente `clipador` (não bloqueante).

**Modo Palavra Completa adicionado (07/09/2026, change OpenSpec `add-full-word-extraction-mode`):** a pedido do usuário, um toggle novo na aba **Enviar Vídeo** do Clip Studio ("Modo Palavra Completa") permite, por submissão, escolher entre gerar Shorts (comportamento padrão, inalterado) ou extrair **um único clipe contínuo** cobrindo toda a pregação de um culto — do início ao fim da mensagem, excluindo abertura/avisos/dízimo/louvor/encerramento, mantendo o formato original do vídeo (sem crop 9:16) e atenuando música/teclado de fundo via filtro ffmpeg (`highpass=f=200,lowpass=f=3000`, best-effort — sem separação de voz real, decisão consciente dado que a VPS é Alpine hardened sem Python). Este é um pipeline **inteiramente separado** do de Shorts:

- **Clip Studio:** `Submission.mode` (novo enum `SHORTS`/`PALAVRA_COMPLETA`, default `SHORTS`), toggle em `SubmitForm.tsx`, propagado por `dispatcher.ts`/`n8n-client.ts` até o webhook de ingestão. A aba Vídeos passa a listar também o clipe de Palavra Completa (badge "Palavra Completa" quando `sourceFolder === "PalavraCompleta/Cortes"`), e o poller de status (`isOriginalArchived`) checa a pasta de arquivamento correta conforme o `mode`.
- **n8n — workflow de ingestão** (`Clip Studio — Integração N8N`, `mfqp4D5HKs0MNhv1`): ganhou um node IF ("Rotear por Modo") logo após o download via `yt-dlp` — o ramo `SHORTS` continua **byte-a-byte idêntico** ao que já existia (confirmado via `get_workflow_details`); o ramo `PALAVRA_COMPLETA` sobe o vídeo para `Videos-Cortes/PalavraCompleta` (pasta criada sob demanda, idempotente) e dispara o novo workflow em vez de "Blocos". Os webhooks `clip-studio/clips` e `clip-studio/videos/check-archived` (mesmo workflow) foram estendidos para também listar/checar a saída da Palavra Completa.
- **Novo workflow n8n "YouTube Palavra Completa — Extração Contínua"** (`knWdWza8AxNht0oJ`, publicado): construído nos mesmos moldes de "Blocos" até a transcrição (Whisper.cpp `large-v3-turbo -t 6`, "Mesclar Pausas Curtas", download via wget streaming) — **crucialmente, usa a MESMA trava de execução compartilhada** (`/home/node/.n8n-files/.processing.lock`) que "Blocos", para nunca rodar 2 whisper.cpp simultâneos na VPS de 6 vCPUs (mesmo bug já documentado em 13/07/2026, agora entre workflows diferentes em vez de duas execuções do mesmo workflow). Diverge depois da transcrição: uma única chamada de IA (`GPT — Limites da Palavra`) reaproveita o texto da regra de exclusão de 5 fases já validado em "Blocos" para devolver só `{sermonStart, sermonEnd, title, reason}`; "Montar Clipe Único" monta 1 clipe sem teto de 180s (só uma duração mínima de sanidade, 60s) e retorna `[]` sem erro quando a IA não encontra pregação (`sermonStart`/`sermonEnd` nulos); o corte final reaproveita o snap de silêncio simétrico e o threshold dinâmico de "Blocos", sem o crop 9:16, com o filtro de áudio adicional. Tem fila própria (`Videos-Cortes/PalavraCompleta`), self-chaining e Schedule Trigger próprio (a cada 30min, mais frequente que os 6h de "Blocos" porque compete pelo mesmo lock e precisa reprocessar mais rápido quando ele libera).
- **Validação local feita antes de publicar:** todos os Code nodes (novos e alterados, nos dois workflows) tiveram a sintaxe confirmada com `new Function()` contra o conteúdo real já armazenado em produção (não uma cópia reescrita à mão); os dois nodes mais críticos ("Mesclar Pausas Curtas PC", "Montar Clipe Único") também foram testados em runtime com dados fictícios realistas; todos os comandos bash novos/alterados passaram em `sh -n`; o build do Clip Studio (`next build`) e o lint (`eslint`) rodaram limpos após as mudanças de schema Prisma/API/UI.
- **Não testado nesta sessão:** (1) o filtro de áudio (`highpass`/`lowpass`) não foi calibrado contra um trecho real com música/teclado de fundo conhecido — não havia amostra disponível; recomenda-se validar com o agente `clipador` ou ouvido humano no primeiro vídeo real. (2) nenhuma execução real de ponta a ponta foi disparada (custaria whisper.cpp/ffmpeg/OpenAI reais) — falta um smoke test com um vídeo curto de teste em modo Palavra Completa, e confirmar que uma submissão em modo Shorts logo antes/depois continua funcionando sem regressão.

**Única opção em uso no gerador HTML: Opção 3 (Blocos).** As Opções 1 (Semântico) e 2 (Simples) não estão sendo usadas e não precisam ser mantidas.
