...
- **Não testado nesta sessão:** (1) nenhuma execução real de ponta a ponta foi disparada em nenhum dos dois workflows n8n (custaria whisper.cpp/ffmpeg/OpenAI reais) — falta um smoke test com um episódio curto de teste em modo Podcast, e outro com upload de arquivo direto. (2) o código do Clip Studio (schema, rota nova, UI, badge) **ainda não foi deployado em produção** — os workflows n8n já estão publicados e ativos, mas o usuário não consegue usar a feature via UI até o deploy acontecer (mesmo fluxo já usado antes nesta sessão: copiar arquivos pra VPS, `docker compose up -d --build`, aplicar migration). (3) confirmar que uma submissão em modo Shorts ou Palavra Completa, disparada perto de um teste de Podcast, continua funcionando sem regressão. (4) calibração fina dos critérios de "destaque de podcast" nos prompts — só validável com episódios reais.
